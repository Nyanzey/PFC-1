...
def convert_string_to_list_of_lists(input_string):
    if input_string == '':
        return []
    # Split the string by semicolon to separate the lists
    list_strings = input_string.split(';')
    
    # Process each list string
    result_list = []
    for list_string in list_strings:
        # Remove any leading/trailing whitespace and split by comma
        float_list = [float(item) for item in list_string.strip().split(',')]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt', type=str, required=True, help='The prompt value')
    parser.add_argument('--phrase', type=str, required=True, help='The phrase value')
    parser.add_argument('--location', type=str, required=True, help='The location value')
    args = parser.parse_args()
    
    promptInput = args.prompt
    phraseList = split_phrases(args.phrase)
    locationList = convert_string_to_list_of_lists(args.location)
    ...
    meta_list = [ 
        ...
        #dict(
            #ckpt = "OUTPUT/zerobetaexp/tag00/checkpoint_00010000.pth",
            #prompt = promptInput,
            #phrases = phraseList,
            #locations = locationList,
            #alpha_type = [0.3, 0.0, 0.7],
            #save_folder_name="generation_betazero"
        #),

        dict(
            ckpt = "../gligen_checkpoints/diffusion_pytorch_model_keypoints.bin",
            prompt = promptInput,
            locations = [],  # from id=18150 val set in coco2017k
            alpha_type = [0.3, 0.0, 0.7],
            save_folder_name="generation_original"
        )
    ]
